...
trace v2 の規則に従い、イベントの `sequence` は1から欠番なく増加します。各フレームの先頭イベントは `event_offset + 1` です。同じ実行内の確定済み item ID の値は変更せず、同じ値の再確定でも `values` に再送しません。
...
通知は約100msごとのポーリングで変更があった場合に送り、複数の commit を1フレームにまとめることがあります。サーバーは `Observe().SnapshotSince(cursor)` で追加イベント・値だけを取得します。変更なしの差分取得は O(1)・割当なし、変更時は追加データのサイズに比例し、毎回の全履歴コピーや全値の走査を行いません。グラフは初回だけ、`Outputs()` は完了時だけ取得します。[runtime の差分 API](../runtime.md#確定データを差分で読む)も参照してください。`ui-kit` 自体は通信・待ち時間・Go runtime に依存しません。
