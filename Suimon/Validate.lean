...
def Program.callsAcyclic (p : Program) : Bool :=
...
def validateBody (p : Program) (at_ : String) : Body → Except String Unit
  | .function id => check (p.function? id).isSome s!"{at_}: unknown function {id}"
...
def validateTask (p : Program) (at_ : String) (c : Concurrency) (task : TaskSpec) : Except String Unit := do
  let at_ := s!"{at_} task {task.name}"
  check (!task.name.isEmpty) s!"{at_}: empty task name"
  p.validateBody at_ task.body
  let input ← need (p.bodyInput task.body) s!"{at_}: unknown body"
...
def validateConnection (p : Program) (w : Workflow) (c : Connection) : Except String Unit := do
...
def validateEntry (p : Program) (w : Workflow) (e : Entry) : Except String Unit := do
...
def validatePlacement (p : Program) (w : Workflow) (pl : Placement) : Except String Unit := do
  let at_ := s!"{w.id}.{pl.name}"
  let incoming := w.incoming pl.name
  match pl.control with
  | .call body => p.validateBody at_ body
  | .branch judge arms =>
    check (p.judge? judge).isSome s!"{at_}: unknown judge {judge}"
  ...
  | .waitStream _ => pure ()
  | .merge _ => check (!incoming.isEmpty) s!"{at_}: Merge needs input connections"
  | .concurrency c =>
  ...
  let expected ← need (p.inputType pl.control) s!"{at_}: unknown reference"
...
def validateWorkflow (p : Program) (w : Workflow) : Except String Unit := do
...
/-- Structural checks of §14. `run` executes only programs accepted here. --/
def validate (p : Program) : Except String Unit := do
...
def Program.WellFormed (p : Program) : Prop := p.validate = .ok ()
