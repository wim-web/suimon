...
it('inspects a placement with its definition, connections, invocations and results', () => {
  ...
  const html = renderToStaticMarkup(<PlacementInspector program={p} workflow="users" placement="perUser" state={s} run={[]} values={values} onSelectRun={() => undefined} onOpenWorkflow={() => undefined} />);
  ...
  const entry = renderToStaticMarkup(<PlacementInspector program={p} workflow="users" placement="fetchAllUsers" state={s} run={[]} values={values} />);
  expect(entry).toContain('&lt;tenant&gt;');
  expect(entry).not.toContain('<tenant>');
  expect(entry).toContain('Stream&lt;User&gt;');
  const definition = renderToStaticMarkup(<PlacementInspector program={p} workflow="users" placement="all" />);
  expect(definition).toContain('endpoint');
  expect(definition).not.toContain('Invocations');
});
...
it('shows task outputs as transformed, failed or pending, and each List under its execution', () => {
  ...
  const failed = renderToStaticMarkup(<PlacementInspector program={p} workflow="users" placement="perUser" state={b} run={[]} values={valueIndex(p, b, recordValues(recordTransitions(records('users-b'))))} />);
  ...
  const running = renderToStaticMarkup(<PlacementInspector program={p} workflow="users" placement="perUser" state={c} run={[]} />);
...
it('lists runs as a tree and records with their commit state and relation', () => {
  ...
  const transitions = recordTransitions(parseRecords(lines.slice(0, 13).join('\n') + '\n'));
  const panel = renderToStaticMarkup(<RecordPanel transitions={transitions} program={p} state={s} />);
  ...
  const filtered = renderToStaticMarkup(<RecordPanel transitions={transitions} program={p} state={s} filter={{ run: [], placement: 'perUser' }} />);
  expect(filtered.match(/sui-record-row/g)?.length).toBe(1);
  expect(renderToStaticMarkup(<RecordPanel transitions={[]} program={p} />)).toContain('No execution records');
  const log = parseRecordLog(recordText('users-c'));
  const torn = renderToStaticMarkup(<RecordPanel transitions={recordTransitions(log.records)} tail={log.tail} program={p} state={state('users-c')} />);
...
it('composes the workbench from a program alone or with a state and records', () => {
  const p = program('users');
  expect(renderToStaticMarkup(<WorkflowWorkbench program={p} />)).toContain('No runtime state');
  const html = renderToStaticMarkup(<WorkflowWorkbench program={p} state={state('users-a')} records={records('users-a')} title="Users" />);
